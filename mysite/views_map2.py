...
def addMessagesToDb(request):
    ...
    computerSorry = "I am sorry that X images assigned to me were counted as misidentifications. It was not your fault. "

    noPromise = ""
    optimisticPromise = "I promise to do better next time."
    realisticPromise = "I cannot do better than this."


    # Group 1 - 3
    #Low : noPromise, optimisticPromise, realisticPromise
    groupId += 1
    MessagePool.objects.create(groupId = groupId, groupName = "lowReliability-noPromise",
        groupReliability = "low", message = computerSorry + noPromise)
    groupId += 1
    MessagePool.objects.create(groupId = groupId, groupName = "lowReliability-optimisticPromise",
        groupReliability = "low", message = computerSorry + optimisticPromise)
    groupId += 1
    MessagePool.objects.create(groupId = ++groupId, groupName = "lowReliability-realisticPromise",
        groupReliability = "low", message = computerSorry + realisticPromise)


    # Group 4 - 6
    #High : noPromise, optimisticPromise, realisticPromise
    groupId += 1
    MessagePool.objects.create(groupId = groupId, groupName = "highReliability-noPromise",
        groupReliability = "high", message = computerSorry + noPromise)
    groupId += 1
    MessagePool.objects.create(groupId = groupId, groupName = "highReliability-optimisticPromise",
        groupReliability = "high", message = computerSorry + optimisticPromise)
    groupId += 1
    MessagePool.objects.create(groupId = ++groupId, groupName = "highReliability-realisticPromise",
        groupReliability = "high", message = computerSorry + realisticPromise)
...
def test2(request):
    ...
    countArray = []
    for obj in Score.objects.order_by().values('mTurkId').distinct():
        #return HttpResponse("mTurkId = %s" %(obj['mTurkId']))
        totalScore = 0
        totalBonus = 0
        roundPlayed = 0
        for roundNo in range(1,6):
            q = Score.objects.filter(mTurkId = obj['mTurkId']).filter(roundNumber = roundNo)
            if q.count() == 0:
                break
            roundScore = q[0].roundScore
            roundPlayed += 1
            #totalBonus += round(round(roundScore/2, 2)*0.004, 2)
            totalScore += roundScore/2

            #return HttpResponse("roundScore = %s" %roundScore)
        if roundPlayed == 5:
            countArray.append({'totalScore':totalScore,'totalBonus':round(totalScore*0.004, 2), 'mTurkId':obj['mTurkId']})


    template = 'bonus.html'
    return render(request, template, {'array': countArray})
...
def test4(request):
    ...
    #hash = 7c24c082bc5a
    #enc = eJzTyCkw5ApzNA4yMfM3D7d0Co9y5iow4krUAwBUZAZX
    #mTurkId = decode_data(hash,enc)[0]
    #groupId = getGroupIdFromMTurkId(mTurkId)

    #return HttpResponse("hash = %s, enc = %s mTurkId = %s"%(hash, enc, mTurkId))

    #mTurkId = 'A3R46O7W9BWZC'
    #hash, enc = encode_data([mTurkId])
    #return HttpResponse("hash = %s enc=%s" %(hash, enc))
